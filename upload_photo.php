<?php
require_once __DIR__ . '/db.php';
require_once __DIR__ . '/functions.php';


$item_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

$stmt = $pdo->prepare("
    SELECT *
    FROM items
    WHERE id = :id
");
$stmt->execute([':id' => $item_id]);
$item = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$item) {
    http_response_code(404);
    echo "指定された装置が見つかりません。";
    exit;
}

$errors = [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $caption = trim($_POST['caption'] ?? '');

    if (!isset($_FILES['photo']) || $_FILES['photo']['error'] === UPLOAD_ERR_NO_FILE) {
        $errors[] = '写真ファイルを選択してください。';
    } else {
        $file = $_FILES['photo'];

        if ($file['error'] !== UPLOAD_ERR_OK) {
            $errors[] = 'アップロードに失敗しました。エラーコード: ' . $file['error'];
        } else {
            $max_size = (int)$config['photo_max_size'];

            if ($file['size'] > $max_size) {
                $errors[] = '写真ファイルは' . format_file_size_limit($max_size) . '以下にしてください。';
            }

            $finfo = new finfo(FILEINFO_MIME_TYPE);
            $mime_type = $finfo->file($file['tmp_name']);

            $allowed = [
                'image/jpeg' => 'jpg',
                'image/png'  => 'png',
                'image/gif'  => 'gif',
                'image/webp' => 'webp',
            ];

            if (!isset($allowed[$mime_type])) {
                $errors[] = 'JPEG, PNG, GIF, WebP の画像ファイルだけ登録できます。';
            }
        }
    }

    if (count($errors) === 0) {
        $photo_dir = $config['photo_directory'];

        if (!is_dir($photo_dir)) {
            if (!mkdir($photo_dir, 0700, true)) {
                $errors[] = '写真保存用ディレクトリを作成できません。';
            }
        }

        if (count($errors) === 0) {
            $ext = $allowed[$mime_type];
            $random = bin2hex(random_bytes(8));
            $stored_name = 'item_' . $item_id . '_' . date('Ymd_His') . '_' . $random . '.' . $ext;
            $dest = $photo_dir . '/' . $stored_name;

            if (!move_uploaded_file($file['tmp_name'], $dest)) {
                $errors[] = '写真ファイルを保存できませんでした。';
            } else {
                chmod($dest, 0600);

                $stmt = $pdo->prepare("
                    INSERT INTO item_photos
                        (item_id, original_name, stored_name, mime_type, file_size, caption, uploaded_at)
                    VALUES
                        (:item_id, :original_name, :stored_name, :mime_type, :file_size, :caption, :uploaded_at)
                ");
                $stmt->execute([
                    ':item_id' => $item_id,
                    ':original_name' => $file['name'],
                    ':stored_name' => $stored_name,
                    ':mime_type' => $mime_type,
                    ':file_size' => $file['size'],
                    ':caption' => $caption,
                    ':uploaded_at' => date('Y-m-d H:i:s'),
                ]);

                header('Location: item.php?id=' . urlencode($item_id));
                exit;
            }
        }
    }
}
?>
<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <title>写真を登録 - <?php echo h($item['asset_tag']); ?> - <?php echo h(system_name()); ?></title>
    <link rel="stylesheet" href="style.css">
</head>
<body>

<h1>写真を登録</h1>

<div class="menu">
    <a href="item.php?id=<?php echo h($item['id']); ?>">詳細へ戻る</a>
    <a href="index.php">一覧へ戻る</a>
</div>

<div class="box">
    <h2>対象装置</h2>
    <table>
        <tr>
            <th>管理番号</th>
            <td><?php echo h($item['asset_tag']); ?></td>
        </tr>
        <tr>
            <th>名称</th>
            <td><?php echo h($item['name']); ?></td>
        </tr>
        <tr>
            <th>型番</th>
            <td><?php echo h($item['model']); ?></td>
        </tr>
        <tr>
            <th>メーカー</th>
            <td><?php echo h($item['manufacturer']); ?></td>
        </tr>
    </table>
</div>

<div class="box">
    <h2>写真の登録方法</h2>

    <p>
        新しい写真をアップロードするか、すでに他の装置に登録されている写真を利用できます。
    </p>

    <div class="menu">
        <a href="reuse_photo.php?id=<?php echo h($item['id']); ?>">既存装置の写真を利用</a>
    </div>
</div>

<?php if (count($errors) > 0): ?>
    <div class="errors">
        <ul>
            <?php foreach ($errors as $e): ?>
                <li><?php echo h($e); ?></li>
            <?php endforeach; ?>
        </ul>
    </div>
<?php endif; ?>

<div class="box">
    <h2>新しい写真をアップロード</h2>

    <form method="post" enctype="multipart/form-data">
        <table>
            <tr>
                <th>写真ファイル</th>
                <td>
                    <input type="file" name="photo" accept="image/*" required>
                    <div class="note">
                        JPEG, PNG, GIF, WebP に対応しています。最大2MBです。
                    </div>
                </td>
            </tr>
            <tr>
                <th>説明</th>
                <td>
                    <input type="text" name="caption" value="<?php echo h($_POST['caption'] ?? ''); ?>">
                    <div class="note">
                        例：前面パネル、背面コネクタ、銘板、付属品など
                    </div>
                </td>
            </tr>
        </table>

        <p>
            <button type="submit">アップロードして登録</button>
        </p>
    </form>
</div>

</body>
</html>
